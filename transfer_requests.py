...
def Advise(A_info, B_info, skip_empty=True, not_early='', not_late='99:99', return_html=False):
    pt = PrettyTable()
    pt._set_field_names('接驳方案 时间 总时间 接驳时间'.split(' '))
    for a in A_info:
        at1 = a['出发时间']
        at2 = a['到达时间']
        for b in B_info:
            ...
            if at2 < bt1 and at1 > not_early and bt2 < not_late:
                ...
                pt.add_row(['%s -> %s' % (a['车次'], b['车次']), '%s-%s' % (at1, bt2), '%d:%d' % (h,m), 'Waiting %d mins' % waiting_time])
    if return_html:
        htmlstr = '<table border="1" width="500" align="center">\n    <caption>换乘方案</caption>\n    ' + pt.get_html_string()[12:]
        htmlstr = htmlstr.replace('td', 'td align="center"')
        return htmlstr
    else:
        print(pt)
                
def To_school(date='2018-12-01', start='端州', not_early='', not_late='99:99', return_html=False):
    ...
    htmlstr = Advise(A_info, B_info, not_early=not_early, not_late=not_late, return_html=return_html)
    if return_html:
        return htmlstr


# path: 广州南 --A--> 肇庆东 - 鼎湖东 --B--> 端州
def To_home(date='2018-12-01', terminal='端州', not_early='', not_late='99:99', return_html=False):
    # date: yyyy-mm-dd
...
def get_transfer_html(From_station=None, To_station=None, Date='yyyy-mm-dd', nearly='00:00', nlate='99:99'):
    ...
    if From_station in ['肇庆', '端州']:
        htmlstr = To_school(date=Date, start=From_station, not_early=nearly, not_late=nlate, return_html=True)
    elif From_station in ['广州南']:
        htmlstr = To_home(date=Date, terminal=To_station, not_early=nearly, not_late=nlate, return_html=True)
    return htmlstr
